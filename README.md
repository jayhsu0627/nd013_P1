...
* **Higher level lane detection**, with left **HSVcolor** and left/right **lane recognition**.
...
## Goals
* **Basic Lane Finding Pipeline** by combining the basic _Heper Function_, result shows in below:
...
* **draw_line** function improvement with fully extented lines and left/right lane is treated differently 
...
>Like the same tool in Photoshop, it turns(filter) the image looks more blur. As a result, it filter out small obejcts in the view meanwhile keeps the image processing mainly focus on bigger object. A sub-function `gaussian_blur` is defined. Within it, `cv2.GaussianBlur` is used.
